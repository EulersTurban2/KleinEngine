...
namespace Engine::Scene {
    ...
    void UniformGrid::insert(uint32_t entity, const glm::vec3& position) {
        glm::ivec3 cell = getCellCoords(position);
        mCells[cell].push_back({entity, position});
        mEntityCellMap[entity] = cell;
    }

    void UniformGrid::remove(uint32_t entity) {
    ...
    void UniformGrid::update(uint32_t entity, const glm::vec3& position) {
    ...
    std::vector<uint32_t> UniformGrid::query(const glm::vec3& position, float radius) {
        ...
        // Inclusive bounds: the query sphere reaches into the max cells too.
